...
├── scripts/
│   ├── make-icons.swift        # CoreGraphics/CoreText 渲染图标（原创"译"字标识）
...
- 图标为本项目原创：`scripts/make-icons.swift` 用 CoreGraphics/CoreText 直接渲染的"译"字标识
  （深蓝圆角底板 + 白色"译"字），**不含**上游 PDFMathTranslate 的任何美术资源。
