...
图形是一个**实心填充的 ∞，被两个圆形挖空**（`infinityRibbon` + `fillPath(using: .evenOdd)`）：
没有文字、没有外框、没有描边。它是 PDF2ZH（PDF → 中文）"无损往返"的直觉表达。

**用填充而不是描边是这里最关键的决定。** 描边版本（早期实现）在 22pt 下挨着系统那些实心图标
像一根头发丝；填充后才有同样的块面重量。而"设计感"由**两个镂空孔**提供——孔洞把实心块切分成
折返的带子，中心因此通透，整体是"丝带"的意味。

孔洞之所以可行、而内部线条不可行：**孔是形状，线不是**。同样一层"内部结构"，孔洞随尺寸等比缩放
依然成立，而 1px 的线在 22pt 下会糊掉（早期试过在外框内加螺旋线，正是这样失败的）。

**外轮廓由四段三次贝塞尔构成一个闭合环**，两瓣在中心跨过一小段竖直区间相接（`waistHalf = 3% 宽`）：
腰部收成零高的尖点会在 22pt 下细到几乎断开，留一小段才既保持交叉结实、又读起来是连续的一条带子。

参数（均为画布比例）：

| 参数 | 取值 | 作用 |
|---|---|---|
| `width` | 0.96 | 横向总宽；几乎用满状态栏的高度预算 |
| `lobeRadiusRatio` | 0.60 | 环的纵向半径占半宽之比；再扁两瓣退化成"蝴蝶结"，再高 22pt 放不下 |
| `controlPull` | 0.52 | 控制点收拢程度；0.46 时腰部过尖，0.56 时瓣形变方 |
| `waistHalf` | 0.03 | 腰部竖直半高，见上 |
| `holeOffset` | 0.29 | 孔心距中心，占总宽之比 |
| `holeRadius` | 0.175 | 孔径，占总宽之比 |

**App 图标**是同一套几何的放大版：圆角底板用蓝色渐变（不是平涂，平涂会显得像占位图），
白色 ∞ 按底板宽度 80% 填充、同样 even-odd 挖孔，孔里透出蓝底。
...
- 双纽线参数方程（`x = a·cosθ/(1+sin²θ)`）画外轮廓：数值上光滑，但填充后腰部内凹成"沙漏"、
  四角带尖，不是饱满的 ∞，故改用贝塞尔外轮廓。
- 描边 + 内螺旋线（模仿厚框内部再走一层螺线）：22pt 下线条糊成一片。**要内部结构就用孔，别用线。**
- 把圆孔换成沿环轴压扁、旋转 38° 的椭圆（想要更明显的"扭转带子"感）：观感确实更"设计"，
  但 22pt 下椭圆的长轴端会贴到轮廓边，读起来像轮廓被啃掉两块，不如正圆干净。已回退。
- 参数化螺旋复刻"从中心绕出的带子"（极坐标渐开线、对数螺线、Catmull-Rom 平滑、显式贝塞尔、
  `addArc` 组合、通道内折返，共二十余轮迭代）：**全部失败**，失败模式有两种且互相矛盾——
  通道窄则两个环各留一个"蚊香"、中心不连通；通道宽则线穿过外轮廓把 ∞ 劈成两半。
  参考图那条 S 形丝带的端点落点与曲率是手工调出来的设计，用参数化曲线逼近不了。
  **结论：这条路不要再走了**，除非能拿到该图形的矢量路径数据（一组现成的贝塞尔控制点）。

若要再改这个图标，先确认新方案在 **22pt**、浅色与深色菜单栏下都成立，再动手。

另一条教训：本项目的参考图解析工具曾把**透明背景误判为墨迹**（RGBA 图里透明像素 RGB 全 0，
按亮度判据必然判成"黑"），导致"扫描参考图反推几何"的尝试全部建立在错误数据上。
