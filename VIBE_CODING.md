...
**结论先行：真实百分比无法获得，因此图标表示"正在翻译"，而不是进度条。** 这不是取巧，
是查证后的唯一诚实做法，证据如下。

#### 为什么 `/api/tasks` 的 progress 永远是 0

server 用正则从子进程输出里抠进度：

```python
MAIN_PROGRESS_RE  = r"\btranslate\s+[^a-z\(\)\r\n]*?(\d+)/(\d+)\b"   # 语义：translate 后只能跟符号和数字
STEP_PROGRESS_RE  = r"(.+?)\(\d+/\d+\)\s+.*?(\d+)/(\d+)"
```

而 pdf2zh_next 实际打印的是 `Translate Paragraphs (1/1)  ━━━━━ 373/…`——阶段名和它自己的
`(1/1)` 夹在 `translate` 与数字之间，被 `[^a-z\(\)]` 排除，于是**永远匹配不上**。

#### 为什么换数据源也拿不到百分比

- `/events` 只是把同一份 `/api/tasks` 每秒推一次，`progress` 坏在哪都一样坏。
- 直接解析 server 日志（`ActivityLogReader`）能拿到真实行，但**分母不可靠**：
  - `Translate Paragraphs … 373/…`：整次翻译只刷新 2 次，分母恒为 `…`（tqdm 在窄 PTY 下截断）
  - `translate ━━━ 0/100`：这个**主**进度条的分母多数时候是数字 100（实测 152 次 vs `…` 44 次），
    但当前值在并行批次期间一直停在 0、结尾才跳到 100，所以仍不是平滑进度

#### 曾考虑并**否决**的方案：量进度条长度

思路是绕开被截断的分母，改用几何量——"100% 对应的条形长度是固定的，量当前长度即可"。
方向看起来更稳，但**数据不支持**，实测三种长度与进度完全无关：

| 条形长度 | 出现次数 | 同时对应的计数 |
|---|---|---|
| 4 | 37 | `10/…` |
| 5 | 95 | **`0/…`、`5/…`、`100/…`、`167/…`、`373/…`** |
| 40 | 152 | 恒为 `0/100` |

长度 5 同时对应 0 和 373 —— 若长度表示比例则不可能。而且全部进度行**没有任何一行把已完成段与
未完成段分开**（`U+2500` 出现 0 次），不存在可量的"实心段"。

原因：tqdm 在**非 TTY 输出**下退化为固定占位条，不绘制比例。所以这条路不是实现难度问题，
而是**数据源里没有这个信息**。若日后要恢复真进度，唯一途径是让 server 给子进程一个足够宽的
PTY（`utils/execute.py` 的 `child_cols`），使 tqdm 输出真正的比例条；本项目刻意不修改第三方
代码，故未采用。
...
| 项 | 做法 |
|---|---|
| 图标 | 有任务时绿色**呼吸**（α 在 0.35…1.0 间以 1.6s 周期变化），完成后满绿 4 秒再回常态 |
| 菜单 | 显示阶段名与真实计数，如 `翻译中：翻译 已处理 37/100` 或 `翻译中：Translate Paragraphs：已处理 373` |
| 正百分比 | **只有 server 真的给出 >0 的 progress 时才显示**；否则不编造 |

呼吸而非"部分填充"是刻意的：部分填充会暗示一个我们并不掌握的进度位置。呼吸只声明"在工作"。

#### 日志解析的两个实现要点

1. **主进度条与阶段进度条必须区分**。两者都以 `translate` 开头；判据是 `translate` 与第一个
   `━` 之间**只能是空格和条形符号**——`Translate Paragraphs (1/1)` 因此被排除，否则阶段计数
   （分母恒为 `…`）会遮蔽主进度条（分母通常是数字）。
2. **解析用字符串切分而不是正则**。进度条由 U+2501 绘制、计数在条形之后，写一个同时处理
   "非数字但允许换行"的正则很容易在 Python→Swift 的转义边界上出错（实际踩过）；
   切分是显式且可测的。
3. 所有可选捕获组都要先判 `NSNotFound`——未参与匹配的组其 range 是 `NSNotFound`，
   直接 `substring` 会抛异常（实际踩过）。
4. **必须要求行内存在条形字符**。否则日志自己的时间戳会被当成进度：`[09/17/26 22:55:44] INFO …`
   含 "09/17"，通用模式会读成"9 之 17"，于是每次空闲轮询都报出一个幽灵任务。tqdm 画的进度行
   一定有条形，日志头一定没有——这是可靠的区分特征。
5. 主进度条的判据要允许**任意非字母数字字符**出现在 `translate` 与条形之间。曾用显式白名单
   （空格/`━`/`═`），结果漏掉 tqdm 的终点标记 `╸`(U+2578)，`translate ╸━━━━ 10/…` 这类行被
   降级到通用路径、阶段名解析成空。
