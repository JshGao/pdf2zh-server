...
图形就是一个**加粗描边的 ∞**：没有外框、没有文字、没有内部细节。它是 PDF2ZH（PDF → 中文）
"无损往返"的直觉表达，而单个符号在 22pt 下的清晰度远好于任何"字 + 图形"的组合。

**几何由四段三次贝塞尔构成一个闭合环**（`infinityPath`）。两瓣共用同一个腰部交点，让交叉点由
几何自然产生而不是靠两根线叠出来——这样笔画在腰部干净地压合，读起来是一条连续丝带。

**笔画刻意做得很粗**（画布 19.5%），这是本图标最重要的一个决定。按"正确"的图标笔画比例画出来
的细线，在 22pt 菜单栏里挨着系统那些实心图标会像一根头发丝；加粗到近五分之一画布后才有同样的
块面重量。代价是**任何内部细节都不可读**——曾试过在外框内加螺旋线（模仿"莫比乌斯带"式图标），
在 22pt 下糊成一团，遂放弃。曲线本身就是全部。

四个参数（均为画布比例）：

| 参数 | 取值 | 作用 |
|---|---|---|
| `halfWidth` | 0.375 | 横向半宽；受笔画宽约束——0.195 的笔画下墨迹已横占 94.5%，再宽就会把轮廓挤出画布 |
| `lobeHalfHeight` | 0.265 | 环的纵向半高；压得再扁两瓣就退化成带折角的方块（"蝴蝶结"），放大则 22pt 状态栏放不下 |
| `controlPull` | 0.420 | 控制点收拢程度；太小掐细腰部，太大则瓣形变方 |
| 笔画宽 | 0.195 | 与系统图标的块面重量相当（见上） |

**App 图标**是圆角底板上一个居中的白色 ∞，底板用蓝色渐变（不是平涂，平涂会显得像占位图），
∞ 按底板宽度 78% 绘制，笔画沿用同一个 19.5% 比例，使两个图标处于同一视觉语言。
...
- 双纽线参数方程画 ∞：形状正确但必须**填充**才能闭合，填充后两瓣实心、在菜单栏尺寸下过重。
- 细描边 + 内螺旋（厚框内部再走一层螺线）：22pt 下细节糊成一片，违背"不要设计感"的方向。
