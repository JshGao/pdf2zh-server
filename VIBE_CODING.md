...
图形是一个**实心填充的 ∞，被两个倾斜椭圆挖空**（`infinityRibbon` + `fillPath(using: .evenOdd)`）：
...
**孔是倾斜椭圆而不是正圆**：把椭圆沿每个环的轴向压扁并旋转，剩下的材质才读起来像一条扭转的带子，
而不是"甜甜圈上戳了两个洞"。倾斜按左右镜像，保证整体关于竖直轴对称。
...
| `holeOffset` | 0.25 | 孔心距中心，占总宽之比 |
| `holeRadiusX / Y` | 0.134 / 0.086 | 椭圆的轴向 / 横向半轴，占总宽之比 |
| `holeTilt` | 38° | 椭圆相对水平轴的倾角，左右镜像 |
...
- 参数化螺旋复刻"从中心绕出的带子"（极坐标渐开线、对数螺线、Catmull-Rom 平滑、显式贝塞尔，
  共十余轮迭代）：**全部失败**。失败模式有两种且互相矛盾——螺旋扫过角小则两个环各留一个
  "蚊香"、中心不连通；扫过角大则线穿过外轮廓把 ∞ 劈成两半。参考图那条曲线的流动感来自
  上游设计，用数学曲线逼近不了。**最终用倾斜椭圆镂空达到等价的"扭转带子"观感。**
  若后人要再试螺旋：先确认它能同时满足"不破坏外轮廓"和"中心连通"这两条，否则直接放弃。
